...
/* Straight to the app's login screen now, not the app root: the app has no
   separate signup screen (it is a mode of the same form on /login), so
   sending a signed-out visitor through the root's splash-then-redirect first
   only costs time. The allowlisted attribution still survives the hop. */
export default async function SignupPage({ searchParams }: { searchParams: Promise<SearchParamsRecord> }) {
